...
_circuit = None
_bitCache = ''
# Default to the local simulator provider so functions work without explicitly
# calling `set_provider_as_IBMQ()` first. This also makes passing an empty
# token (as suggested in the README) behave as "use local backend".
provider = qiskit.BasicAer
def set_provider_as_IBMQ(token: str | None = None):
    """
    Sets the backend provider to IBMQ with the given account token. Will fall back
    to the local (simulated) provider if no token is provided or if loading the
    IBMQ account fails.

    Parameters:
        token (str | None): Account token on IBMQ. If falsy (None or empty
            string), the local simulator provider (`qiskit.BasicAer`) will be used.
    """
    global provider
    # Treat falsy token (None or empty string) as local/simulator request.
    if not token:
        provider = qiskit.BasicAer
        return

    # Try to save/load the IBMQ account and get the provider. If anything goes
    # wrong, fall back to the local simulator and print a helpful message.
    try:
        ...
        provider = qiskit.BasicAer
        print("Warning: could not initialise IBMQ provider. Falling back to local simulator. Error:", e)
